...
Renderer.prototype._renderAFrames = function(streamTime){
    var
        startTime, endTime, aSample, sampleTime;
    ...
    while(this.audioQueue.length && (streamTime+2000)>=(this.audioQueue[0].pts/48)){
        ...
        if(streamTime<=startTime){
            if(!this._prevTime) sampleTime = this.actx.currentTime+(startTime-streamTime)/1000;
            else sampleTime = this._prevTime + aSample.tlen/1000;
            aSample.start(sampleTime);
            this._prevTime = sampleTime;
            this.scheduled.push(aSample);
        }
        else {
            if(!this.scheduled.length &&  (streamTime - startTime) > 100){
                console.log('resync',streamTime,this.currentTime, startTime);
                this._startedAt = 0;
                this._prevTime = 0;
...
Renderer.prototype._renderer = function(){
  ...
  //  if(!(_cntr++%100)) console.log(this.frames.length,this.audioQueue.length,this.scheduled.length);
    ...
    window.requestAnimationFrame(this._RAHandler);
