...
if(!memcpy.binding){
    console.warn('WARNING from nw-f2cwa-play: memcpy module is probably not compiled with nw-gyp. please compile memcpy module with nw-gyp and place compiled file  memcpy.node to '+memcpy.path+' for your nw version to get maximum performance!');
}
    ...
    this._videoData = function(data){
       ...
       // console.log(data.length,currPos,freeBuffers.length,buffer.length);
        ...
        if((toWrite)<data.length) {
            this._videoData(data.slice(toWrite<<2));
        }
    };
