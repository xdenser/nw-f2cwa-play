...
Player.prototype.openSrc = function(fileName,skip,vsize){
    ...
    args =  args.concat(['-i',fileName],videoArgs,'-vf',vsize?('[in]showinfo, scale=-1:'+vsize+'[out]'):'showinfo',this.videoDest,audioArgs,this.audioDest);
...
Player.prototype._videoData = function(data){
    ...
    function fillRGB(rgb,w,h){
        var x, y,
            ...
            cy,cy2,cy24,co;
        for(y=0;y<h;y++)
        for(x=0,cy=y*w,cy2 =(y >> 1)*w2+sz, cy24 = cy2+sz4,co=y*w*4 ;x<w;x++){
            Y = buffer[cy + x];
            U = buffer[cy2 + (x >> 1)];
            V = buffer[cy24 + (x >> 1)]
            C = (Y - 16)*298 + 128;
            D = U - 128;
            E = V - 128;
            rgb[co+x*4] = clamp((C + 409*E)>>8); // R
            rgb[co+x*4+1] = clamp((C - 100*D - 208*E)>>8); // G
            rgb[co+x*4+2] = clamp((C + 516*D)>>8); // B
            rgb[co+x*4+3] = 0xFF; // A
        }
    }
    ...
    this._videoData = function(data){
        ...
        if(!buffer) buffer = new Buffer(this.width*this.height*1.5);
        var
            bufferRGB = this.currBuffer || (currPos=0,this.freeBuffers.shift()) || (this.metaReceived ? new window.Uint8Array(this.width*this.height*4):null),
            toWrite;

        if(!bufferRGB) return currPos+=data.length;
        if(!bufferRGB.reuse) bufferRGB.reuse = function(){
        ...
        data.copy(buffer,currPos,0,toWrite);
        currPos += toWrite;
       // console.log(data.length,currPos,freeBuffers.length,buffer.length);

        if(currPos == buffer.length){
            this.currBuffer = null;
            bufferRGB.frameNumber = this._currFrameNum;//this.frameCount++;
            bufferRGB.framePts = this._currFramePts;
            fillRGB(bufferRGB,this.width,this.height);
            this.emit('video',bufferRGB);
            currPos = 0;
        } else this.currBuffer = bufferRGB;
...
Player.prototype._audioData = function(data){
   ...
   // console.log('adata',this.aframeCount++);
    var abuf = this.currABuf || this.audioBuffers.shift() || new AudioBuffer(2/this.fps);
